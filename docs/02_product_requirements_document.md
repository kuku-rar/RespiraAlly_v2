...
**更新背景**: 客戶於 2025-10-19 提出三項新需求，經 Linus 式技術評估後，接受 CR-001 和 CR-003（簡化版），拒絕 CR-002。以下記錄完整評估結果與整合細節。
...
| **CR-002** | 語音 CAT 量表 | ❌ **拒絕/延後** | - | ~~128h~~ | Phase 2+ | 全系統 |
| **CR-003** | 營養評估 KPI (簡化版) | ✅ **接受** | **P1** | **56h** | **Sprint 3** | Backend, Database, Frontend |
| **總計** | **接受的新需求** | - | - | **66h** | - | - |

**工時影響**:
- Sprint 2 增加 10h (4.2.9 - 4.2.11)
- Sprint 3 增加 56h (5.5 新模組)
- **總開發時程延長約 8 工作天** (66h ÷ 8h/day = 8.25 天)
...
#### 需求描述
**客戶訴求**: "病患填寫的每日健康日誌數據（水分攝取、運動時間、服藥次數等）可能存在錯誤輸入（如水分攝取 9999ml），需要系統提供合理性驗證與警告機制。"
...
**驗證規則表**:
| 欄位 | 資料類型 | 合理範圍 | 異常警告閾值 | 錯誤阻擋閾值 | 提示訊息 |
|------|---------|---------|-------------|-------------|----------|
| 水分攝取 (ml) | Integer | 500 - 3000 | < 300 or > 4000 | < 0 or > 10000 | "一般建議每日 1500-2500ml，您的輸入為 {value}ml，請確認。" |
| 運動時間 (min) | Integer | 0 - 180 | > 240 | > 500 | "超過 4 小時運動較不常見，請確認。" |
| 服藥次數 (次) | Integer | 0 - 10 | > 12 | > 20 | "服藥次數異常，請確認。" |
| 體重 (kg) | Float | 30 - 150 | < 25 or > 180 | < 10 or > 300 | "體重數值異常，請確認。" |
| 痰量 (mL) | Integer | 0 - 500 | > 600 | > 1000 | "痰量數值較高，請確認。" |

**允收標準** (UAT):
1. 後端: Pydantic model 拋出 `ValidationError` 當數值超過錯誤閾值
2. 後端: API 回傳 `warnings` 欄位當數值超過警告閾值
3. 前端: 表單送出前顯示警告對話框 (仍可強制提交)
  ...
  "warnings": [
    {
      "field": "water_intake",
      "value": 4500,
      "message": "一般建議每日 1500-2500ml，您的輸入為 4500ml，請確認。"
    }
  ]
}
```
...
### 6.3 CR-002: 語音 CAT 量表 ❌ (拒絕/延後至 Phase 2+)

#### 需求描述
**客戶訴求**: "希望病患能透過語音（如 LINE 語音訊息）完成 CAT 量表填寫，避免長者手動點選困難。"

**技術規格估算**:
- STT 整合 (20h): 語音檔上傳、轉文字、意圖識別
- 多輪對話流程 (40h): CAT 8 題 × 5 選項 = 40 輪對話狀態管理
- 語音導引 TTS (24h): 8 題問題語音生成、選項朗讀
- 語音意圖理解 (24h): "有一點" vs "有點" → 分數映射
- 錯誤重試機制 (12h): 聽不清楚、重新回答邏輯
- 測試與調優 (8h)
- **總計**: **128h** (約 16 工作天)

#### 決策理由 (Linus 式批判)

**這是個假問題 (False Problem)**:
```
CAT 量表只有 8 題，每題 5 個選項，手動點選時間 < 3 分鐘
長者填寫困難？讓我們看數據：
- 現有 LIFF UI 已經是大按鈕設計
- 8 次點擊 vs 8 輪語音對話，哪個更簡單？
- 語音的真正問題：環境噪音、口音、重聽
```

**複雜度爆炸 (Complexity Explosion)**:
```
為了省下 3 分鐘點擊，我們要做：
- 128 小時的開發
- 8 題 × 5 選項 = 40 種狀態機轉換
- STT 錯誤率 10% × 8 題 = 至少 1 題需重試
- 測試成本：8 題 × 5 選項 × 2 種口音 = 80 測試案例

這個複雜度與問題嚴重性完全不匹配。
```

**實用性檢驗 (Practicality Check)**:
```
問題：有多少長者真的因為「點擊困難」而放棄填寫？
現實：零數據支持

Linus 語錄: "Theory and practice sometimes clash. Theory loses. Every single time."
...
#### 替代方案 (Phase 2+ 可考慮)
**如果未來有真實數據證明問題存在，建議**:
- 方案 A: 提供「治療師協助填寫」功能 (8h 開發)
- 方案 B: 使用 LINE 官方的「快速回覆按鈕」優化 UI (4h 開發)
- 方案 C: 整合現有語音助理 (Google Assistant / Siri) 而非自建 (20h)

**結論**: **MVP 階段拒絕此需求，Phase 2+ 根據實際使用數據重新評估。**
