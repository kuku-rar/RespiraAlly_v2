...
class SurveyResponseModel(Base):
    ...
    answers: Mapped[dict] = mapped_column(
        JSONB, nullable=False, comment="Survey answers: {q1: 2, q2: 3, ...}"
    )
    total_score: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Total calculated score"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint("total_score >= 0", name="survey_total_score_check"),
    )
