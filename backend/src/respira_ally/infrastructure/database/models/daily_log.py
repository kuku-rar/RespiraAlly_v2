...
class DailyLogModel(Base):
    ...
    patient_id: Mapped[UUID] = mapped_column(
        ForeignKey("patient_profiles.user_id", ondelete="CASCADE"), nullable=False
    )
    ...
    medication_taken: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, comment="Whether medication was taken (NULL = not recorded)"
    )
    water_intake_ml: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Daily water intake in milliliters (NULL = not recorded)"
    )
    exercise_minutes: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Daily exercise duration in minutes (replaces steps_count)"
    )
    smoking_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Number of cigarettes smoked (COPD risk factor)"
    )

    # Symptoms & Mood
    symptoms: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Reported symptoms")
    ...
    def __repr__(self) -> str:
        return (
            f"<DailyLog(log_id={self.log_id}, patient_id={self.patient_id}, date={self.log_date})>"
        )
